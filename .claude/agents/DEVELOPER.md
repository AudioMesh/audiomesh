...
This agent ruleset defines the standard end-to-end workflow for an AI Agent handling development tasks in the **AudioMesh** project.
...
- If required, update or write the corresponding `README.md` file following the guidelines and templates defined in the [readme skill](file:///Users/mr.robot/z-stash/AudioMesh/audiomesh/.claude/skills/readme/SKILL.md).

### 3. Create Jira Ticket

- Automatically create a Jira ticket in the `AUD` project.
...
- **IMPORTANT**: You must ask the user which Epic the ticket should be linked to. Provide the user with these options:
  - `1` for **Infrastructure & Modernization** (`AUD-1`)
  - `2` for **Web App Features** (`AUD-2`)
  - `3` for **Visual Excellence** (`AUD-3`)
  - `4` for **Localization & Accessibility** (`AUD-4`)
  - `5` for **Desktop Core Experience & Platform Distribution** (`AUD-5`)
  - `6` for **Mobile Core Experience & Platform Distribution** (`AUD-6`)
- Wait for the user's response. Once the user provides the Epic choice, link the newly created ticket to the corresponding Epic using `parent: "EPIC-KEY"` (e.g., `parent: "AUD-1"`).
- Follow all standard naming and template guidelines from `.claude/skills/jira/SKILL.md` (e.g., prefixing `[Web]`, `[Core]`, `[Design]`, `[Desktop]`, or `[Mobile]` to the title, and applying the corresponding label).
...
- Target the `AudioMesh/audiomesh` repository.
